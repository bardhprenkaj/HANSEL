...
class GCounteRGANExplainer(Explainer):
    ...
    def fit(self, oracle: Oracle, dataset : Dataset, fold_id=0):
        ...
        if os.path.exists(explainer_uri):
            ...
            for i in range(self.n_labels):
                self.__fit(oracle, dataset, fold_id, desired_label=i)
    ...
    def __fit(self, oracle : Oracle, dataset : Dataset, fold_id, desired_label=0):
        generator_loader, discriminator_loader = self.transform_data(dataset, fold_id, class_to_explain=desired_label)
        countergan = self.explainers[desired_label]
    ...
    def __pad_matrices(self, matrices):
        max_rows = max(matrix.shape[0] for matrix in matrices)
        max_cols = max(matrix.shape[1] for matrix in matrices)

        padded_rows = (max_rows + 3) // 4 * 4
        padded_cols = (max_cols + 3) // 4 * 4
        
        padded_matrices = []
        for matrix in matrices:
            rows_to_pad = padded_rows - matrix.shape[0]
            cols_to_pad = padded_cols - matrix.shape[1]
            
            padded_matrix = np.pad(matrix, ((0, rows_to_pad), (0, cols_to_pad)), mode='constant')
            padded_matrices.append(padded_matrix)
        
        return np.array(padded_matrices), padded_rows
